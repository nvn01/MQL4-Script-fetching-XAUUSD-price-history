//+------------------------------------------------------------------+
//|                                             ExportXAUUSDMultiTF.mq4 |
//|  This script exports XAUUSD data from January 1, 2025 onward for   |
//|  eight different timeframes:                                     |
//|    - XAU_15m_data.csv   => 15-minute (M15)                         |
//|    - XAU_1Month_data.csv=> 1-month (MN1)                            |
//|    - XAU_1d_data.csv    => Daily (D1)                               |
//|    - XAU_1h_data.csv    => Hourly (H1)                              |
//|    - XAU_1w_data.csv    => Weekly (W1)                              |
//|    - XAU_30m_data.csv   => 30-minute (M30)                          |
//|    - XAU_4h_data.csv    => 4-hour (H4)                              |
//|    - XAU_5m_data.csv    => 5-minute (M5)                            |
//+------------------------------------------------------------------+
#property strict
...
void ExportData(string symbol, int timeframe, string fileName, datetime startTime)
{
   ...
   // Open the CSV file (located in MQL4/Files folder)
   ...
   // Loop through all bars from the oldest to the most recent
   for(int i = totalBars - 1; i >= 0; i--)
   {
      datetime timeStamp = iTime(symbol, timeframe, i);
      
      // Only process bars on or after the startTime
      if(timeStamp < startTime)
         continue;
      
      // Format the date string (date and minutes)
...
void OnStart()
{
   // Define the symbol (update if needed) and start date (January 1, 2025)
   string symbol = "XAUUSD";
   datetime startTime = StrToTime("2025.01.01 00:00");
   
   // Define file names for each timeframe (CSV extension added)
   string fileNames[8] = {
   ...
   // Loop through each timeframe and export its data
   for (int i = 0; i < 8; i++)
   {
